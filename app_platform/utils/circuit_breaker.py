...
class CircuitBreaker:
    ...
    def __init__(
        ...
        *,
        ...
        self._opened_at: float = 0.0 # Opened at
        self._half_open_probe_inflight = False # Half open probe inflight
        self._threshold = max(min_fails, int(failure_threshold)) # Threshold
        self._window_s = float(window_seconds) # Window size
        self._half_open_after = float(half_open_after_s) # Half open after
        self._backoff = backoff_fn or (lambda n: min(1.0, 0.05 * (2 ** max(0, n - 1)))) # Backoff function
        self._corr_fn = correlation_key_fn or (lambda exc: getattr(exc, "__class__", type("")) .__name__ if exc else "generic") # Correlation function
    ...
    def _trip_if_needed(self, now: float) -> None:
        """Trip the circuit breaker if needed."""
    ...
    def wrap_call(self, fn: Callable[[], Any], *, max_tries: int = 1) -> Any:
        """Wrap a call to the circuit breaker."""
