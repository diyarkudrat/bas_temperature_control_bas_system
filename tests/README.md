# BAS System Testing Framework Docs

Welcome to the documentation for the BAS System testing framework. These docs mirror the clarity and organization style used in `docs/auth`, focusing on concise explanations, consistent sections, and practical examples.

## Index

- [01 — Overview](./01-overview.md)
- [02 — Architecture](./02-architecture.md)
- [03 — Fixtures](./03-fixtures.md)
- [04 — Markers](./04-markers.md)
- [05 — Plugins](./05-plugins.md)
- [06 — Business Rules](./06-business-rules.md)
- [07 — Contract Testing](./07-contract-testing.md)
- [08 — Running & CI](./08-running-and-ci.md)
- [09 — Troubleshooting](./09-troubleshooting.md)
- [Framework Guide (Deep Dive)](./framework-guide.md)
- [10 — Style & Conventions](./10-style-and-conventions.md)

## Quick Links

- [Simple Examples: Adding New Tests](./adding-tests-examples.md)