...
| `server/auth/models.py` | Ensure `user_id`, `username`, `role`, `expires_at`, `tenant_id` fields. | ✅ **COMPLETED** |
| `server/auth/services.py` | Branch to Firestore-backed Users/Sessions on flags; enforce re-login at cutover. | ✅ **COMPLETED** |
| `server/auth/managers.py` | Integrate stores for login/logout; structured audit hooks. | ✅ **COMPLETED** |
