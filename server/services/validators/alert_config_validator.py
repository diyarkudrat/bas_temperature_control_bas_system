...
def _check_version(cfg: Dict[str, Any], errors: List[str]) -> None:
    version = cfg.get("version")
    if not isinstance(version, str) or not SEMVER_RE.match(version):
        errors.append("version: must be a semantic version string like '1.0.0'")


def _check_default_channels(routing: Dict[str, Any], errors: List[str]) -> None:
...
def _check_severity_routes(routing: Dict[str, Any], errors: List[str]) -> None:
    severity_routes = routing.get("severity_routes", {}) or {}
...
def _check_tenants(routing: Dict[str, Any], errors: List[str]) -> None:
    tenants = routing.get("tenants", {}) or {}
...
def _check_providers(cfg: Dict[str, Any], routing: Dict[str, Any], errors: List[str]) -> None:
    providers = cfg.get("providers", {}) or {}
...
def _check_limits(cfg: Dict[str, Any], errors: List[str], warnings: List[str]) -> None:
...
def _semantic_checks(cfg: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    _check_version(cfg, errors)
    routing = cfg.get("routing", {}) or {}
    _check_default_channels(routing, errors)
    _check_severity_routes(routing, errors)
    _check_tenants(routing, errors)
    _check_providers(cfg, routing, errors)
    _check_limits(cfg, errors, warnings)
