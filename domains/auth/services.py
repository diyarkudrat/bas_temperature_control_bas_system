...
class PasswordPolicy:
    """Domain wrapper for password validation policy."""

    @staticmethod
    def validate(password: str) -> tuple[bool, str]:
        try:
            from app_platform.utils.auth import validate_password_strength
            return validate_password_strength(password)
        except Exception as e:  # fallback deny if utilities unavailable
            logger.warning(f"password policy check failed: {e}")
            return False, "Password policy check failed"
