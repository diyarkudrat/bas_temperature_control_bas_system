...
class _InProcessHub:
	...
	def __init__(self, heartbeat_interval_s: float, subscriber_queue_maxsize: int) -> None:
		"""Initialize the in-process hub."""

		self._heartbeat_interval_s = heartbeat_interval_s # heartbeat interval
		self._subscriber_queue_maxsize = subscriber_queue_maxsize # subscriber queue maxsize
		self._subscribers: Dict[str, Queue[str]] = {} # subscribers
		self._subscribers_lock = threading.RLock() # subscribers lock
		self._next_event_id = 0 # next event id
