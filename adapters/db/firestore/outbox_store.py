...
class OutboxRepository(TimestampedRepository):
    ...
    def enqueue(
        ...
        *,
        ...
        try:
            ...
            return OperationResult[str](success=True, data=event.event_id)
    ...
    def get(self, event_id: str) -> OperationResult[OutboxEvent]:
        ...
        try:
            ...
            if not doc.exists:
                return OperationResult[OutboxEvent](success=False, error="Outbox event not found", error_code="NOT_FOUND")

            entity = create_outbox_event(doc.to_dict())
            entity.id = doc.id

            return OperationResult[OutboxEvent](success=True, data=entity)
        except Exception as exc:  # noqa: BLE001
