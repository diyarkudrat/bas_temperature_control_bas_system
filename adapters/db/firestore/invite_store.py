...
class InviteRepository(TimestampedRepository):
    ...
    def __init__(self, client: firestore.Client):
        """Initialize the invite repository."""
    ...
    def get_by_id(self, invite_id: str) -> OperationResult[Invite]:
        ...
        try:
            ...
            if not doc.exists:
                return OperationResult[Invite](success=False, error="Invite not found", error_code="NOT_FOUND")

            invite = create_invite(doc.to_dict())
            invite.id = doc.id

            return OperationResult[Invite](success=True, data=invite)
        except Exception as exc:  # noqa: BLE001
            self._handle_firestore_error("get invite", exc)

    def get_active_invite(self, tenant_id: str, email: str) -> Optional[Invite]:
        """Get an active invite by tenant ID and email."""
